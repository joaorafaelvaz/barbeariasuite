...
import { renderMessage } from "@/lib/wahasend/waha-client";
import { Button } from "@/components/ui/button";
import FileDropzone from "./file-dropzone";
import type { Attachment } from "@/lib/wahasend/types";
  ...
  return (
    <div className="space-y-6">
      ...
      {!isGroupMode && variables.length > 0 && (
        <div>
          <p className="text-xs text-text-muted mb-2">Inserir vari&aacute;vel:</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div className="bg-surface-hover border border-border-subtle rounded-lg px-4 py-3 text-sm text-text-primary min-h-[200px] whitespace-pre-wrap">
            {getPreview() || <span className="text-text-muted italic">Sua mensagem aparecer&aacute; aqui...</span>}
          </div>
        </div>
      </div>
