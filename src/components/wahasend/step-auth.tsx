...
import { createSession, getQrCode, getSessionStatus, getGroups } from "@/lib/wahasend/waha-client";
import { v4 as uuidv4 } from "uuid";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import type { WhatsAppGroup } from "@/lib/wahasend/types";
...
export default function StepAuth() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-text-primary">Autentica&ccedil;&atilde;o WhatsApp</h2>
      ...
      <div className="max-w-xs">
        <Label htmlFor="send-interval">
          {isGroupMode ? "Intervalo entre grupos (segundos)" : "Intervalo entre mensagens (segundos)"}
        </Label>
        <Input
          id="send-interval"
          type="number"
      ...
      <div className="flex flex-col items-center gap-4 py-4">
        {status === "idle" || status === "error" ? (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-text-muted">Criando sess&atilde;o...</p>
          </div>
        ) : status === "waiting_qr" ? (
              ...
              {groupPollActive ? (
                <>
                  <div className="text-center">
                    <p className="text-sm text-text-primary font-medium">Aguardando sincroniza&ccedil;&atilde;o...</p>
                    <p className="text-xs text-text-muted mt-1">
                      O WhatsApp est&aacute; sincronizando seus grupos. Buscando automaticamente a cada 5 segundos.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-accent border-t-transparent rounded-full animate-spin" />
                    <span className="text-xs text-text-muted">
                      Tentativa {groupPollCount + 1}/{MAX_GROUP_POLLS} &mdash; ~{remainingSeconds}s restantes
                    </span>
                  </div>
                  <Button
                    variant="secondary"
