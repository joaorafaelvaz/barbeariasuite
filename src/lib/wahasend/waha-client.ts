export async function createSession(name: string): Promise<void> {
  const res = await fetch("/api/wahasend/sessions", {
    method: "POST",
...
export async function deleteSession(name: string): Promise<void> {
  await fetch(`/api/wahasend/sessions?name=${encodeURIComponent(name)}`, {
    method: "DELETE",
  });
}

export async function getSessionStatus(name: string): Promise<string> {
  const res = await fetch(`/api/wahasend/sessions?name=${encodeURIComponent(name)}`);
...
export async function getQrCode(session: string): Promise<string> {
  const res = await fetch(`/api/wahasend/auth/qr?session=${encodeURIComponent(session)}`);
  if (!res.ok) {
...
export async function checkNumber(session: string, phone: string): Promise<boolean> {
  const res = await fetch(
    `/api/wahasend/contacts/check?session=${encodeURIComponent(session)}&phone=${encodeURIComponent(phone)}`
  );
...
export async function sendTextMessage(
  ...
  const res = await fetch("/api/wahasend/messages/text", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function sendImageMessage(
  ...
  const res = await fetch("/api/wahasend/messages/image", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function sendFileMessage(
  ...
  const res = await fetch("/api/wahasend/messages/file", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getGroups(session: string): Promise<WAGroup[]> {
  const res = await fetch(`/api/wahasend/groups?session=${encodeURIComponent(session)}`);
  if (!res.ok) {
